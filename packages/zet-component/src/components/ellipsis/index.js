...
const EllipsisText = ({ text, length, tooltip, fullWidthRecognition, ...other }) => {
  ...
  if (process.env.NODE_ENV === 'development') {
    const isValid = React.isValidElement(text);
    if (isValid) {
  ...
  if (textLength <= length || length < 0) {
    // adaptor intl begin
    if (process.env.NODE_ENV === 'development') {
      return <span title={title} {...other}>{text}</span>;
    }
    // adaptor intl end
  ...
  if (tooltip) {
    return (
      <Tooltip overlayStyle={TooltipOverlayStyle} title={text} placement='topLeft'>
        <span title={title}>
          {displayText}
          {tail}
        </span>
...
export default class Ellipsis extends Component {
  ...
  render() {
    ...
    if (isSupportLineClamp) {
      ...
      return tooltip ? (
        <Tooltip overlayStyle={TooltipOverlayStyle} title={children} placement='topLeft'>
          {node}
        </Tooltip>
      ) : (
        node
